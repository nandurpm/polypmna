...
export default function Landing() {
 ...
 const hasLesson = true; // lessons exist for most codes
 return (
 <motion.div
 key={`${r.code}-${i}`}
 ...
 className="group rounded-xl border border-border bg-card p-3 hover:border-primary/20 transition-all"
 >
 ...
 {pdf && (
 <a
 href={`/pdf?url=${encodeURIComponent(pdf.pdfUrl)}&title=${encodeURIComponent(cleanTitle(pdf.title))}&code=${r.code}`}
 ...
 {pdf && (
 <a
 href={getPdfDownloadUrl(r.code)}
 download
 target="_blank"
 rel="noopener noreferrer"
 className="inline-flex items-center gap-1 rounded-md bg-emerald-500/10 px-2 py-1 text-[11px] font-medium text-emerald-600 hover:bg-emerald-500/20 transition-colors"
 >
 ...
 {hasLesson && (
 <a
 href={`/lesson?code=${r.code}&title=${encodeURIComponent(r.name)}`}
 className="inline-flex items-center gap-1 rounded-md bg-violet-500/10 px-2 py-1 text-[11px] font-medium text-violet-600 hover:bg-violet-500/20 transition-colors"
 >
 <FileText className="h-3 w-3" /> Read Lesson
 </a>
 )}
 </div>
 </div>
 ...
 <motion.h1 className="text-2xl sm:text-3xl font-bold tracking-tight leading-tight">
 Your Polytechnic Study Space
 </motion.h1>
 <motion.p className="mt-2 text-sm sm:text-base text-muted-foreground leading-relaxed max-w-2xl">
 Browse {subjects.length.toLocaleString()} subjects across {programmes.length} departments,
 access {pdfs.length.toLocaleString()} study notes, {papers.length.toLocaleString()} question papers,
 and lesson content — all organized by department and semester.
 </motion.p>
 <motion.div className="mt-5 flex flex-wrap gap-3">
 ...
 <motion.div initial={{ opacity: 0, y: 14 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.4, ease: [0.22, 1, 0.36, 1] }}>
 ...
 {/* ─── Featured Study Notes (real PDFs) ─── */}
 <section className="pb-8">
 <motion.div initial={{ opacity: 0, y: 14 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.4, ease }}>
 ...
 {featuredPdfs.map((mat, idx) => (
 <motion.div
 key={mat.code}
 initial={{ opacity: 0, y: 10 }}
 whileInView={{ opacity: 1, y: 0 }}
 viewport={{ once: true }}
 transition={{ delay: Math.min(idx * 0.03, 0.3), duration: 0.3, ease }}
 ...
 className="flex-1 inline-flex items-center justify-center gap-1.5 rounded-lg bg-primary px-3 py-1.5 text-xs font-medium text-primary-foreground hover:bg-primary/90 transition-all"
 >
 ...
 target="_blank"
 rel="noopener noreferrer"
 className="flex-1 inline-flex items-center justify-center gap-1.5 rounded-lg border border-border px-3 py-1.5 text-xs font-medium text-foreground hover:bg-muted transition-all"
 >
 ...
 Revision 2026 &amp; 2021 · Kerala Polytechnic Curriculum · {subjects.length.toLocaleString()} subjects · {pdfs.length.toLocaleString()} PDFs
